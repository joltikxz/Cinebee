...
let cards = [
    ...
    {
        background:
            "//external-content.duckduckgo.com/iu/?u=https%3A%2F%2Fwallpaperaccess.com%2Ffull%2F2776321.jpg&f=1&nofb=1&ipt=a8848f195949343166c2865585d1e6d3c5fcd97b8f43f27b246c5304f9f1d176&ipo=images",
        title: "TENET",
        description:
            "Watch this incredible film made by some incredible people!"
    },
    {
        background:
            "//external-content.duckduckgo.com/iu/?u=http%3A%2F%2Fhdqwalls.com%2Fwallpapers%2Fmad-max-fury-road.jpg&f=1&nofb=1&ipt=d60c885af6d377eda23f3eff630d92000ae094668f90a11e1768ab6b56813a9a&ipo=images",
...
function playTrailer(movieId) {
    ...
    let video = document.getElementById('trailer-video');
    let trailerSource = document.getElementById('trailer-source');

    // Here we map movieId to the trailer URL
    let trailerUrl = getTrailerUrl(movieId);
    trailerSource.src = trailerUrl;

    video.load(); // Load the new trailer
    video.play(); // Play the trailer

    // After 45 seconds, pause the video
    setTimeout(function() {
        video.pause(); // Pause the video after 45 seconds
...
// Function to get the trailer URL based on the movie ID
function getTrailerUrl(movieId) {
    const trailers = {
        "movie-1": "trailers/movie1-trailer.mp4", // Local trailer video
        "movie-2": "trailers/movie2-trailer.mp4",
        "movie-3": "trailers/movie3-trailer.mp4",
        "movie-4": "trailers/movie4-trailer.mp4",
        "movie-5": "trailers/movie5-trailer.mp4",
        "movie-6": "trailers/movie6-trailer.mp4",
        "movie-7": "trailers/movie7-trailer.mp4",
        "movie-8": "trailers/movie8-trailer.mp4",
        "movie-9": "trailers/movie9-trailer.mp4",
        "movie-10": "trailers/movie10-trailer.mp4"
        // Add more movies as needed
    };

    return trailers[movieId] || ""; // Return the URL for the movie
}
